...
describe('Test generate error', () => {
    it("should return simple error", () => {
        ...
        expect(generatedError).toHaveProperty("code", "UNEXPECTED_INTERNAL_SERVER_ERROR");
    ...
    it("should return customized error message", () => {
        let generatedError = errorUtil.generateError(
            { message: "message with {firstVar},  {secondVar} and {missedVar}." },
            { firstVar: "first value", secondVar: "second value" }
        );

        expect(generatedError).toHaveProperty("message", "message with first value,  second value and '#N/A'.");
    });
